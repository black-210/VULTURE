#include "vulture_c_features.h"

#include <errno.h>
#include <math.h>
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#define SHA256_MASK 0xFFFFFFFFu

typedef struct {
    uint32_t state[8];
    uint64_t bitlen;
    unsigned char data[64];
    size_t datalen;
} SHA256_CTX;

static const uint32_t k[64] = {
    0x428a2f98u, 0x71374491u, 0xb5c0fbcfu, 0xe9b5dba5u,
    0x3956c25bu, 0x59f111f1u, 0x923f82a4u, 0xab1c5ed5u,
    0xd807aa98u, 0x12835b01u, 0x243185beu, 0x550c7dc3u,
    0x72be5d74u, 0x80deb1feu, 0x9bdc06a7u, 0xc19bf174u,
    0xe49b69c1u, 0xefbe4786u, 0x0fc19dc6u, 0x240ca1ccu,
    0x2de92c6fu, 0x4a7484aau, 0x5cb0a9dcu, 0x76f988dau,
    0x983e5152u, 0xa831c66du, 0xb00327c8u, 0xbf597fc7u,
    0xc6e00bf3u, 0xd5a79147u, 0x06ca6351u, 0x14292967u,
    0x27b70a85u, 0x2e1b2138u, 0x4d2c6dfcu, 0x53380d13u,
    0x650a7354u, 0x766a0abbu, 0x81c2c92eu, 0x92722c85u,
    0xa2bfe8a1u, 0xa81a664bu, 0xc24b8b70u, 0xc76c51a3u,
    0xd192e819u, 0xd6990624u, 0xf40e3585u, 0x106aa070u,
    0x19a4c116u, 0x1e376c08u, 0x2748774cu, 0x34b0bcb5u,
    0x391c0cb3u, 0x4ed8aa4au, 0x5b9cca4fu, 0x682e6ff3u,
    0x748f82eeu, 0x78a5636fu, 0x84c87814u, 0x8cc70208u,
    0x90befffau, 0xa4506cebu, 0xbef9a3f7u, 0xc67178f2u
};

static uint32_t rotr(uint32_t x, uint32_t n) {
    return (x >> n) | (x << (32u - n));
}

static void sha256_transform(SHA256_CTX *ctx, const unsigned char data[]) {
    uint32_t a, b, c, d, e, f, g, h;
    uint32_t m[64];
    uint32_t t1, t2;
    int i;

    for (i = 0; i < 16; ++i) {
        m[i] = ((uint32_t)data[i * 4] << 24) |
               ((uint32_t)data[i * 4 + 1] << 16) |
               ((uint32_t)data[i * 4 + 2] << 8) |
               ((uint32_t)data[i * 4 + 3]);
    }
    for (i = 16; i < 64; ++i) {
        uint32_t s0 = rotr(m[i - 15], 7) ^ rotr(m[i - 15], 18) ^ (m[i - 15] >> 3);
        uint32_t s1 = rotr(m[i - 2], 17) ^ rotr(m[i - 2], 19) ^ (m[i - 2] >> 10);
        m[i] = m[i - 16] + s0 + m[i - 7] + s1;
    }

    a = ctx->state[0];
    b = ctx->state[1];
    c = ctx->state[2];
    d = ctx->state[3];
    e = ctx->state[4];
    f = ctx->state[5];
    g = ctx->state[6];
    h = ctx->state[7];

    for (i = 0; i < 64; ++i) {
        uint32_t s1 = rotr(e, 6) ^ rotr(e, 11) ^ rotr(e, 25);
        uint32_t ch = (e & f) ^ (~e & g);
        uint32_t temp1 = h + s1 + ch + k[i] + m[i];
        uint32_t s0 = rotr(a, 2) ^ rotr(a, 13) ^ rotr(a, 22);
        uint32_t maj = (a & b) ^ (a & c) ^ (b & c);
        uint32_t temp2 = s0 + maj;

        h = g;
        g = f;
        f = e;
        e = d + temp1;
        d = c;
        c = b;
        b = a;
        a = temp1 + temp2;
    }

    ctx->state[0] += a;
    ctx->state[1] += b;
    ctx->state[2] += c;
    ctx->state[3] += d;
    ctx->state[4] += e;
    ctx->state[5] += f;
    ctx->state[6] += g;
    ctx->state[7] += h;
}

static void sha256_init(SHA256_CTX *ctx) {
    ctx->state[0] = 0x6a09e667u;
    ctx->state[1] = 0xbb67ae85u;
    ctx->state[2] = 0x3c6ef372u;
    ctx->state[3] = 0xa54ff53au;
    ctx->state[4] = 0x510e527fu;
    ctx->state[5] = 0x9b05688cu;
    ctx->state[6] = 0x1f83d9abu;
    ctx->state[7] = 0x5be0cd19u;
    ctx->bitlen = 0;
    ctx->datalen = 0;
}

static void sha256_update(SHA256_CTX *ctx, const unsigned char *data, size_t len) {
    size_t i;

    for (i = 0; i < len; ++i) {
        ctx->data[ctx->datalen] = data[i];
        ctx->datalen++;
        if (ctx->datalen == 64) {
            sha256_transform(ctx, ctx->data);
            ctx->bitlen += 512;
            ctx->datalen = 0;
        }
    }
}

static void sha256_final(SHA256_CTX *ctx, unsigned char hash[32]) {
    size_t i;
    uint64_t bitlen = ctx->bitlen + (uint64_t)ctx->datalen * 8;

    ctx->data[ctx->datalen++] = 0x80u;
    if (ctx->datalen > 56) {
        while (ctx->datalen < 64) {
            ctx->data[ctx->datalen++] = 0x00u;
        }
        sha256_transform(ctx, ctx->data);
        ctx->datalen = 0;
    }

    while (ctx->datalen < 56) {
        ctx->data[ctx->datalen++] = 0x00u;
    }

    for (i = 0; i < 8; ++i) {
        ctx->data[56 + i] = (unsigned char)(bitlen >> (8 * (7 - i)));
    }

    sha256_transform(ctx, ctx->data);

    for (i = 0; i < 8; ++i) {
        hash[i * 4] = (unsigned char)((ctx->state[i] >> 24) & 0xffu);
        hash[i * 4 + 1] = (unsigned char)((ctx->state[i] >> 16) & 0xffu);
        hash[i * 4 + 2] = (unsigned char)((ctx->state[i] >> 8) & 0xffu);
        hash[i * 4 + 3] = (unsigned char)(ctx->state[i] & 0xffu);
    }
}

static void sha256_to_hex(const unsigned char hash[32], char hex_digest[65]) {
    static const char *hex = "0123456789abcdef";
    size_t i;

    for (i = 0; i < 32; ++i) {
        hex_digest[i * 2] = hex[(hash[i] >> 4) & 0x0fu];
        hex_digest[i * 2 + 1] = hex[hash[i] & 0x0fu];
    }
    hex_digest[64] = '\0';
}

static int compare_doubles(const void *lhs, const void *rhs) {
    const double left = *(const double *)lhs;
    const double right = *(const double *)rhs;
    if (left < right) {
        return -1;
    }
    if (left > right) {
        return 1;
    }
    return 0;
}

int vulture_load_signal_file(const char *path, SignalBuffer *signal) {
    FILE *fp = fopen(path, "r");
    double value = 0.0;
    size_t capacity = 32;
    size_t count = 0;

    if (signal == NULL || path == NULL) {
        return -1;
    }

    signal->values = NULL;
    signal->count = 0;
    signal->capacity = 0;

    if (fp == NULL) {
        return -1;
    }

    signal->values = (double *)malloc(capacity * sizeof(double));
    if (signal->values == NULL) {
        fclose(fp);
        return -1;
    }

    while (fscanf(fp, "%lf", &value) == 1) {
        if (count == capacity) {
            double *new_values;
            capacity *= 2;
            new_values = (double *)realloc(signal->values, capacity * sizeof(double));
            if (new_values == NULL) {
                free(signal->values);
                signal->values = NULL;
                fclose(fp);
                return -1;
            }
            signal->values = new_values;
        }
        signal->values[count++] = value;
    }

    fclose(fp);

    if (count == 0) {
        free(signal->values);
        signal->values = NULL;
        return -1;
    }

    signal->count = count;
    signal->capacity = capacity;
    return 0;
}

void vulture_free_signal(SignalBuffer *signal) {
    if (signal == NULL) {
        return;
    }
    free(signal->values);
    signal->values = NULL;
    signal->count = 0;
    signal->capacity = 0;
}

int vulture_compute_stats(const SignalBuffer *signal, SignalStats *stats) {
    double sum = 0.0;
    double sum_sq = 0.0;
    double *sorted = NULL;
    size_t i;

    if (signal == NULL || stats == NULL || signal->count == 0 || signal->values == NULL) {
        return -1;
    }

    sorted = (double *)malloc(signal->count * sizeof(double));
    if (sorted == NULL) {
        return -1;
    }
    memcpy(sorted, signal->values, signal->count * sizeof(double));
    qsort(sorted, signal->count, sizeof(double), compare_doubles);

    stats->min = sorted[0];
    stats->max = sorted[signal->count - 1];
    stats->peak_to_peak = stats->max - stats->min;

    for (i = 0; i < signal->count; ++i) {
        sum += signal->values[i];
        sum_sq += signal->values[i] * signal->values[i];
    }

    stats->mean = sum / (double)signal->count;
    stats->rms = sqrt(sum_sq / (double)signal->count);

    {
        double variance_sum = 0.0;
        for (i = 0; i < signal->count; ++i) {
            double diff = signal->values[i] - stats->mean;
            variance_sum += diff * diff;
        }
        stats->variance = variance_sum / (double)signal->count;
        stats->stddev = sqrt(stats->variance);
    }

    if (signal->count % 2 == 0) {
        stats->median = (sorted[(signal->count / 2) - 1] + sorted[signal->count / 2]) / 2.0;
    } else {
        stats->median = sorted[signal->count / 2];
    }

    free(sorted);
    return 0;
}

int vulture_find_peaks(const SignalBuffer *signal, PeakSummary *peak_summary, double threshold) {
    size_t i;
    double mean = 0.0;
    double stddev = 0.0;
    SignalStats stats;

    if (signal == NULL || peak_summary == NULL) {
        return -1;
    }

    if (vulture_compute_stats(signal, &stats) != 0) {
        return -1;
    }

    mean = stats.mean;
    stddev = stats.stddev;

    peak_summary->count = 0;
    if (threshold <= 0.0) {
        threshold = mean + stddev;
    }

    for (i = 1; i + 1 < signal->count; ++i) {
        double current = signal->values[i];
        if (current >= signal->values[i - 1] && current >= signal->values[i + 1] && current >= threshold) {
            if (peak_summary->count < VULTURE_FEATURES_MAX_PEAKS) {
                peak_summary->indices[peak_summary->count] = i;
                peak_summary->amplitudes[peak_summary->count] = current;
                peak_summary->count++;
            }
        }
    }

    return 0;
}

void vulture_generate_report(const SignalStats *stats, const PeakSummary *peaks, char *buffer, size_t buffer_len) {
    if (buffer == NULL || buffer_len == 0) {
        return;
    }

    if (stats == NULL || peaks == NULL) {
        snprintf(buffer, buffer_len, "{\n  \"status\": \"invalid\"\n}\n");
        return;
    }

    snprintf(buffer, buffer_len,
             "{\n"
             "  \"engine\": \"VULTURE_C_FEATURES\",\n"
             "  \"mean\": %.12f,\n"
             "  \"stddev\": %.12f,\n"
             "  \"variance\": %.12f,\n"
             "  \"rms\": %.12f,\n"
             "  \"median\": %.12f,\n"
             "  \"min\": %.12f,\n"
             "  \"max\": %.12f,\n"
             "  \"peak_to_peak\": %.12f,\n"
             "  \"peaks_detected\": %zu,\n"
             "  \"status\": \"stable\"\n"
             "}\n",
             stats->mean,
             stats->stddev,
             stats->variance,
             stats->rms,
             stats->median,
             stats->min,
             stats->max,
             stats->peak_to_peak,
             peaks->count);
}

int vulture_sha256_file(const char *path, char hex_digest[65]) {
    FILE *fp;
    unsigned char buffer[4096];
    unsigned char digest[32];
    SHA256_CTX ctx;
    size_t bytes_read;

    if (path == NULL || hex_digest == NULL) {
        return -1;
    }

    fp = fopen(path, "rb");
    if (fp == NULL) {
        return -1;
    }

    sha256_init(&ctx);
    while ((bytes_read = fread(buffer, 1, sizeof(buffer), fp)) > 0) {
        sha256_update(&ctx, buffer, bytes_read);
    }
    if (ferror(fp) != 0) {
        fclose(fp);
        return -1;
    }
    fclose(fp);

    sha256_final(&ctx, digest);
    sha256_to_hex(digest, hex_digest);
    return 0;
}

int vulture_sha256_buffer(const unsigned char *buffer, size_t length, char hex_digest[65]) {
    unsigned char digest[32];
    SHA256_CTX ctx;

    if (buffer == NULL || hex_digest == NULL) {
        return -1;
    }

    sha256_init(&ctx);
    sha256_update(&ctx, buffer, length);
    sha256_final(&ctx, digest);
    sha256_to_hex(digest, hex_digest);
    return 0;
}

int vulture_run_demo_suite(void) {
    const double synthetic[] = {
        0.0, 1.0, 0.2, 3.0, 0.1, 4.5, 0.3, 2.2, 0.15, 5.0,
        0.2, 1.3, 0.4, 4.0, 0.15, 3.5, 0.1, 2.8, 0.5, 6.0,
        0.35, 1.9, 0.15, 5.5, 0.2, 2.4
    };
    SignalBuffer signal;
    SignalStats stats;
    PeakSummary peaks;
    char report[1024];
    const char *demo_payload = "VULTURE C feature suite demo payload";
    char digest[65];

    signal.values = (double *)malloc(sizeof(synthetic));
    if (signal.values == NULL) {
        return -1;
    }
    memcpy(signal.values, synthetic, sizeof(synthetic));
    signal.count = sizeof(synthetic) / sizeof(synthetic[0]);
    signal.capacity = signal.count;

    if (vulture_compute_stats(&signal, &stats) != 0) {
        free(signal.values);
        return -1;
    }
    if (vulture_find_peaks(&signal, &peaks, 0.0) != 0) {
        free(signal.values);
        return -1;
    }
    vulture_generate_report(&stats, &peaks, report, sizeof(report));

    if (vulture_sha256_buffer((const unsigned char *)demo_payload,
                              strlen(demo_payload),
                              digest) != 0) {
        free(signal.values);
        return -1;
    }

    printf("VULTURE C demo suite\n");
    printf("Report:\n%s", report);
    printf("Detected peaks: %zu\n", peaks.count);
    printf("SHA256(demo_payload): %s\n", digest);

    free(signal.values);
    return 0;
}
